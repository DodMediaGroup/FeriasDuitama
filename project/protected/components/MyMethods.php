...
class MyMethods extends CApplicationComponent
{
	...
	public static function sentMail($subject, $from, $to, $content, $options=array()){
		$fromName = (isset($options['fromName']))?$options['fromName']:'Ouna - OUNASOUVENIRS.COM';
		...
		$body = '<table cellpadding="0" cellspacing="0" width="100%" style="border-collapse:collapse;background:#9096a5;min-width:620px;table-layout:fixed;">'.
			...
			$content.'<br>'.
			'Enviado desde OUNASOUVENIRS.COM<br><br>'.
			'</div></td></tr>'.
			'<tr><td align="center" style="background-color:#eeeeee; color:#444444; font-size:10px; padding-top:5px; padding-bottom:5px;">Derechos reservados Ouna</td></tr>'.
	...
	public static function array_sort_by(&$arrIni, $col, $order = SORT_ASC) {
	    ...
	    {
	    	//Modified to DuitamaPropone
	        //$arrAux[$key] = is_object($row) ? $arrAux[$key] = $row->$col : $row[$col];
	        $arrAux[$key] = is_object($row) ? $arrAux[$key] = count($row->$col) : $row[$col];
